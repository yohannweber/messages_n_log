CLASS lcl_messages_internal IMPLEMENTATION.
  ...
  METHOD add.
    ...
    DATA internal TYPE REF TO data.
    ...
    append_to_internal( internal = internal ).
*        CATCH lcx_e_or_a_type_occurs.    "
*        CATCH lcx_error_occurs.    "
  ...
  METHOD add_string_to_internal.
    ...
    lv_string_length = strlen( message ).

    DO.
      lv_length = nmin( val1 = 220 val2 = lv_string_length ).
      cs_message-message = message+lv_offset(lv_length).
      APPEND cs_message TO et_messages.
      lv_offset = lv_length.
...
*                RAISE EXCEPTION TYPE lcx_error_occurs.
                  RAISE EXCEPTION TYPE cx_sy_move_cast_error.
                ENDIF.
                ASSIGN internal->* TO <fs_bapiret2_t>.
                IF NOT <fs_bapiret2_t> IS ASSIGNED.
*                RAISE EXCEPTION TYPE lcx_error_occurs.
                  RAISE EXCEPTION TYPE cx_sy_move_cast_error.
                ENDIF.
                append_t( it_messages = <fs_bapiret2_t> ).
*                  CATCH lcx_e_or_a_type_occurs.    "
              CATCH cx_sy_move_cast_error.
            ENDTRY.
...
*            RAISE EXCEPTION TYPE lcx_error_occurs.
              RAISE EXCEPTION TYPE cx_sy_move_cast_error.
            ENDIF.
            ASSIGN internal->* TO <fs_bapiret2>.
            IF NOT <fs_bapiret2> IS ASSIGNED.
*            RAISE EXCEPTION TYPE lcx_error_occurs.
              RAISE EXCEPTION TYPE cx_sy_move_cast_error.
            ENDIF.
            append( is_message = <fs_bapiret2> ).
*                  CATCH lcx_e_or_a_type_occurs.    "
        ENDCASE.
    ENDTRY.
...
CLASS lcl_messages_system IMPLEMENTATION.
  ...
  METHOD mapping_to_internal.
    ...
    CREATE DATA internal TYPE bapiret2.
    ASSIGN internal->* TO <fs_bapiret2>.
    IF NOT <fs_bapiret2> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.

    <fs_bapiret2>-id          = sy-msgid.
    <fs_bapiret2>-type        = sy-msgty.
    <fs_bapiret2>-number      = sy-msgno.
    <fs_bapiret2>-message_v1  = sy-msgv1.
    <fs_bapiret2>-message_v2  = sy-msgv2.
    <fs_bapiret2>-message_v3  = sy-msgv3.
    <fs_bapiret2>-message_v4  = sy-msgv4.
...
CLASS lcl_messages_if_message IMPLEMENTATION.
  METHOD mapping_to_internal.
    ...
    IF NOT <fs_bapiret2> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.
    CREATE DATA lr_data TYPE bapiret2_t.
    ASSIGN lr_data->* TO <fs_bapiret2_t>.
    IF NOT <fs_bapiret2_t> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.
    ...
    add_string_to_internal(
      EXPORTING
        message     = lv_message
      IMPORTING
        et_messages = <fs_bapiret2_t>
      CHANGING
        cs_message  = <fs_bapiret2> ).
...
CLASS lcl_messages_bapi_order_rtrn_t IMPLEMENTATION.
  ...
  METHOD mapping_to_internal.
    ...
    FIELD-SYMBOLS : <fs_bapiret2_t>   TYPE bapiret2_t,
    ...
    CREATE DATA internal TYPE bapiret2_t.
    ASSIGN internal->* TO <fs_bapiret2_t>.
    IF NOT <fs_bapiret2_t> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.
    ...
    LOOP AT lt_bapi_order_return ASSIGNING <ls_order_return>.
      MOVE-CORRESPONDING <ls_order_return> TO ls_return.
      IF ls_return-type IS INITIAL.
        ls_return-type = iv_msgty.
      ENDIF.
      APPEND ls_return TO <fs_bapiret2_t>.
    ENDLOOP.
...
CLASS lcl_messages_lif_messages IMPLEMENTATION.
  ...
  METHOD mapping_to_internal.
    ...
    li_messages->get( IMPORTING messages = <fs_bapiret2_t> ).
...
CLASS lcl_messages_string IMPLEMENTATION.
  ...
  METHOD mapping_to_internal.
    ...
    IF NOT <fs_bapiret2_t> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.
    CREATE DATA lr_bapiret2 TYPE bapiret2.
    ASSIGN lr_bapiret2->* TO <fs_bapiret2>.
    IF NOT <fs_bapiret2> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.
...
CLASS lcl_messages_bapiret2_t IMPLEMENTATION.
  METHOD mapping_to_internal.
    ...
    IF NOT <fs_bapiret2_t> IS ASSIGNED.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.
    <fs_bapiret2_t> = messages.
    LOOP AT <fs_bapiret2_t> ASSIGNING <fs_bapiret2>.
      IF <fs_bapiret2>-type IS INITIAL.
        <fs_bapiret2>-type = get_default_msg_type( ).
      ENDIF.
    ENDLOOP.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS:
      ...
      get_table_type_descr      IMPORTING io_tabledescr TYPE REF TO cl_abap_tabledescr
                                ...
                                RAISING   cx_sy_move_cast_error,
      get_ref_type_descr        IMPORTING io_refdescr  TYPE REF TO cl_abap_refdescr
                                EXPORTING eo_typedescr TYPE REF TO cl_abap_typedescr
                                RAISING   cx_sy_move_cast_error,
    ...
    CLASS-DATA: mth_ref_routing TYPE ty_t_ref_routing.
...
CLASS lcl_messages_type IMPLEMENTATION.
  ...
  METHOD class_constructor.
    DATA ls_ref_routing TYPE ty_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_table.
    ls_ref_routing-type_name  = 'BAPIRET2'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_BAPIRET2_T'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.
    ls_ref_routing-way        = 'O'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_table.
    ls_ref_routing-type_name  = 'BDCMSGCOLL'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_BDCMSGCOLL_T'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_struct1.
    ls_ref_routing-type_name  = 'BAPIRET2'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_BAPIRET2'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.
    ls_ref_routing-way        = 'O'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.
    ...
    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_struct1.
    ls_ref_routing-type_name  = 'SYST'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_SYSTEM'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_string.
    ls_ref_routing-type_name  = 'STRING'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_STRING'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_char.
    ls_ref_routing-type_name  = ''.
    ls_ref_routing-class_name = 'LCL_MESSAGES_STRING'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_oref.
    ls_ref_routing-type_name  = 'IF_MESSAGE'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_IF_MESSAGE'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'I'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_oref.
    ls_ref_routing-type_name  = 'LIF_MESSAGES'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_LIF_MESSAGES'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'O'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_table.
    ls_ref_routing-type_name  = 'BAL_S_MSG'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_BAL_T_MSG'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.

    ls_ref_routing-way        = 'O'.
    ls_ref_routing-type_kind  = cl_abap_typedescr=>typekind_char.
    ls_ref_routing-type_name  = 'BALLOGHNDL'.
    ls_ref_routing-class_name = 'LCL_MESSAGES_BALLOGHNDL'.
    INSERT ls_ref_routing INTO TABLE mth_ref_routing.
  ...
  METHOD get_messages_type_n_name.
    ...
    DATA: lo_typedescr  TYPE REF TO cl_abap_typedescr,
          ...
          ls_interfaces TYPE abap_intfdescr,
          li_t100       TYPE REF TO if_message.
    ...
    CASE lo_typedescr->type_kind.
      ...
      WHEN cl_abap_typedescr=>typekind_oref.
        ...
        lo_typedescr ?= cl_abap_typedescr=>describe_by_object_ref( messages ).
        TRY.
            lo_classdescr ?= lo_typedescr.
            " RIGH NOW, we dont manage several interfaces implementations
            IF NOT lo_classdescr->interfaces IS INITIAL.
              READ TABLE lo_classdescr->interfaces INTO ls_interfaces INDEX 1.
              ev_type_name = ls_interfaces-name.
            ELSE.
              ev_type_name = lo_classdescr->get_relative_name( ).
            ENDIF.
          CATCH cx_sy_move_cast_error.
            RETURN.
        ENDTRY.
...
*          EXPORTING
*            the_subject = messages.
**        CL_INSTANCE_DESCRIPTION=>

*        ev_type_name = l_exception_description->class_name.
*        ev_type_name = l_exception_description->.
*        li_t100 ?= messages.
*        ev_type_name = 'IF_MESSAGE'.
        RETURN.
  ...
  METHOD get_ref_type_descr.

    DATA: li_t100 TYPE REF TO if_message.

*    io_refdescr->get_relative_name( ).
...
*        ENDTRY.
*    ENDTRY.
  ENDMETHOD.                    "get_ref_type_descr

  METHOD instance_narrowing.

    DATA : li_messages    TYPE REF TO lif_messages_internal,
           ls_ref_routing TYPE ty_ref_routing.

    READ TABLE mth_ref_routing INTO ls_ref_routing WITH TABLE KEY way       = iv_way
                                                                  type_kind = iv_type_kind
                                                                  type_name = iv_type_name.
    IF NOT sy-subrc IS INITIAL
    OR ls_ref_routing-class_name IS INITIAL.
      RAISE EXCEPTION TYPE cx_sy_move_cast_error.
    ENDIF.

    CREATE OBJECT li_messages TYPE (ls_ref_routing-class_name).
